...
# Helper function to connect to RPC (modified to be reusable)
def connect_to_rpc(rpc_url):
    # Use default RPC URL if rpc_url is None
    if rpc_url is None:
...
async def execute_bebop_wrap_unwrap(
    private_key: str,
    amount_mon: float,
    ...
    try:
        ...
        contract = w3.eth.contract(address=w3.to_checksum_address(wmon_contract_address), abi=contract_abi)
        amount_wei = w3.to_wei(amount_mon, 'ether')
        logs.append(f"Starting Bebop Wrap/Unwrap for {amount_mon} MON | Wallet: {wallet_short}")

        # --- Wrap MON --- #
        logs.append(format_step('wrap', f"Preparing to wrap {amount_mon} MON..."))
        gas_price_wrap = await asyncio.get_event_loop().run_in_executor(None, lambda: w3.eth.gas_price) # Use executor for blocking call
        ...
        tx_wrap = contract.functions.deposit().build_transaction({
            ...
            'value': amount_wei,
            'gas': 50000, # Adjusted gas limit for simple wrap
        ...
        # Fetch nonce again as it has changed after wrapping
        ...
        tx_unwrap = contract.functions.withdraw(amount_wei).build_transaction({
            'from': account.address,
            'gas': 60000, # Adjusted gas limit for simple unwrap
        ...
        # --- Success --- #
        final_message = f"Successfully wrapped and unwrapped {amount_mon} MON."
        ...
        error_message = f"Bebop Wrap/Unwrap failed: {str(e)}"
        logs.append(format_step('error', f"✘ Failed: {error_message}"))
        logs.append(f"TRACEBACK: {tb_str}")
        print(f"ERROR in execute_bebop_wrap_unwrap: {error_message}\nTRACEBACK:\n{tb_str}") # Print to backend console
        return {
            'success': False,
            'message': error_message,
            'wrap_tx_hash': wrap_tx_hash, # Include hash if wrap succeeded before error
            'unwrap_tx_hash': unwrap_tx_hash,
            'logs': logs
        }

# --- Removed original functions: --- #
# load_private_keys, print_border, print_step (replaced by format_step),
# get_mon_amount_from_user, get_random_delay,
# wrap_mon, unwrap_mon (logic moved into execute_bebop_wrap_unwrap),
# run_swap_cycle, run, if __name__ == "__main__"
